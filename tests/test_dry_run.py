...
def test_strategy_dry_run_performs_no_filesystem_ops(tmp_path: Path) -> None:
    target = tmp_path / "proj" / "node_modules"
    target.mkdir(parents=True)
    (target / "package.json").write_text("{}")

    result = TrashStrategy(trash_root=tmp_path / "trash").delete(
        [_item(str(target))], total_size=100, dry_run=True
    )
...
def test_strategy_dry_run_still_reports_blocked_paths(tmp_path: Path) -> None:
    safe = tmp_path / "proj" / "node_modules"
    safe.mkdir(parents=True)

    result = TrashStrategy(trash_root=tmp_path / "trash").delete(
        [_item("/"), _item(str(safe))], total_size=200, dry_run=True
    )
...
def test_delete_items_dry_run_writes_no_metadata(tmp_path: Path) -> None:
